...
"tsep4BO","BottDur","tbottend","tbotBO","tsepBOST","tStopMig","tBotNT","tBotST","tstrNT","tsepNTST")
partablescaled<-cbind(sNe1BO,sNe2BO,sNe3BO,sNe4BO,sNeST,sNe1NT,sNe2NT,sMigBO,sMigNT,sMig56,sMig65,sMig57,sMig75,sMigSTNT,sMigNTST,sMigBOST,sMigSTBO,sNeancBO,rBO,sNeanc1NT,sNeanc2NT,sNeancST,
stsep4BO,stbottend,stsepBOST,stStopMig,stBotNT,stBotST,ststrNT,stsepNTST)
...
for (i in 1:nsims){	
	...
	s[16]<-paste(" -ej ",as.character(stsepNTST[i])," 5 6 ",sep="")
...
#print(li1)
	...
	if (i==1){
		system(paste(li1," | ",cpd,"compute_pd.py -np 7 -nc ",nchr," -w 30 -b 100 -s > ",out,".1.tab",sep=""))
	}else{
		system(paste(li1," | ",cpd,"compute_pd.py -np 7 -nc ",nchr," -w 30 -b 100 -s >> ",out,".1.tab",sep=""))
	}
}
