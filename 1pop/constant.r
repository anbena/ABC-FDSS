#wrapper script to simulate one constant population
#Rscript --vanilla ./constant.r <nchr> <locuslength> <nlociXsim> <rec.rate=1e-8> <nsimulations>
...
#Parameter transformations for ms simulator
tn1<-4*n1*mu*ll
srec<-4*n1*(recomb*(ll-1))

#Output parameters files
partable<-cbind(n1,n1)
colnames(partable)<-c("n1","n1")
partablescaled<-cbind(tn1,tn1,srec)
